...
--[[
	Chameleon theme - originally intended for use with wal.
	Originally Based off copycat-killers multicolor theme.
...
-- beautiful init
beautiful.init(os.getenv("HOME") .. "/.config/awesome/themes/chameleon/theme.lua")
...
local graphics   = "gimp"
local tagnames   = { ":www", ":trm", ":doc", ":med", ":fil", ":etc" }
...
local mytextclock = wibox.widget.textclock(markup(beautiful.color04, "%A %d %B ") .. markup(beautiful.color02, "»") .. markup(beautiful.color06, " %H:%M "))
...
local cpuwidget = lain.widgets.cpu({
    settings = function()
        widget:set_markup(markup(beautiful.color07, "cpu: " .. cpu_now.usage .. "% » "))
    end
})
...
function (widget, args) return  "bat1: ".. args[2] .. "% "
end, 1, 'BAT0')
...
function (widget, args) return  "- bat2: ".. args[2] .. "% » "
end, 1, 'BAT1')
...
function( widget, args)
  return string.format("%.8s » ", args["{ssid}"])
end, 19, "wlp3s0")
--
...
local volume = lain.widgets.alsa({
    settings = function()
        ...
        widget:set_markup(markup(beautiful.color09, "vol: " .. volume_now.level .. "% » "))
    end
})
...
local memwidget = lain.widgets.mem({
    settings = function()
        widget:set_markup(markup(beautiful.color13, "mem: " ..mem_now.used .. "M » "))
    end
})
...
local mpdwidget = lain.widgets.mpd({
    settings = function()
        ...
        widget:set_markup(markup(beautiful.color04, artist) .. markup("#b2b2b2", title))
    end
})
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ altkey }, "p", function() os.execute("scr") end),
...
--    awful.key({ altkey, }, "h", function () fsroot.show(7) end),
   awful.key({ altkey, }, "w", function () myweather.show(7) end),
    ...
    awful.key({ modkey }, "l", function () os.execute("i3lock-fancy") end),
    ...
    awful.key({ modkey }, "r", function ()
        awful.spawn(string.format("dmenu_run -i -q -fn '-benis-lemon-medium-r-normal--10-110-75-75-m-50-iso8859-9' -w 200 -l 3 -x 28 -y 56  -nb '%s' -nf '%s' -sb '%s' -sf '%s'",
